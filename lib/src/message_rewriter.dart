...
String rewriteMessages(String source, String sourceName,
    {forceName: false, useStringSubstitution: false}) {
  var messages = findMessages(source, sourceName, null, forceName);
  messages.sort((a, b) => a.sourcePosition.compareTo(b.sourcePosition));

  var start = 0;
  var newSource = new StringBuffer();
  for (var message in messages) {
    if (message.arguments.isNotEmpty || forceName) {
...
List findMessages(String source, String sourceName,
    [MessageExtraction extraction, forceName = false]) {
